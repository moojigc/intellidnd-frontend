...
class Player extends Table {
	...
	async sync(playerData, otherId) {
		let response;
		if (!playerData && !otherId) response = await this.dbRead();
		else if (otherId) response = await this.find(otherId);
		else response = playerData;
	...
	async checkToken(token, name) {
		let res = await this.find({ token: token, name: name });
		if (res) {
			// Skip dbRead;
