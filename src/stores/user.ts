...
class User implements UserAttributes {
    ...
    subscribe: Writable<Partial<UserAttributes>>["subscribe"];
    set(values: Partial<UserAttributes>) {
    ...
    private _setLs(user: Partial<UserAttributes>) {
        ...
        for (const k in user) {
            if (["token", "fetching", "notification"].includes(k)) {
                continue;
            }
    ...
    private async _getRefreshToken(throwError = false) {
        try {
            const data = await request("/user/session/refresh", "POST", null, {
                data: {
        ...
        try {
            ...
            if (e.response.status == 429) {
                ...
                switch (target) {
                    ...
                    default:
                        this.notify(
                            `You are doing that too many times! Please try again in ${inMinutes} minutes.`,
                            "error"
                        );
                }
            }
            ...
            return [
                {
                    ...
                    inventory: {
                        ...
                        wallet: {
                            ...
                            platinum: 0,
                            update: async (coins) => {
                                const char = await this._request<Character>(
                                    `/characters`,
                                    "POST",
                                    {
                                        data: {
        ...
        const characters: Character[] = chars.map((char) => ({
            ...
            inventory: {
                ...
                removeItem: async (itemId) => {
                    await this._request(
                        `/characters/${char.id}/inventory/${itemId}`,
                        "DELETE"
                    );
                },
                ...
                wallet: {
                    ...
                    getTotalAs: (type?: keyof Coins) => {
                        const { gold, copper, electrum, platinum, silver } =
                            char.inventory.wallet;
    ...
    async signup(details: {
        ...
        const res = await this._Request("/user/signup", "POST", {
            data: details,
        });
    ...
    async login(identifier: string, password: string) {
        const user = await this._Request("/user/login", "POST", {
            data: {
                identifier,
                password,
            },
    ...
    async getCharacters(updates?: Partial<CharacterStatic>) {
        if (updates) {
    ...
    async verifyEmail(email: string, code: string) {
        await this._request(
            `/user/email/${encodeURIComponent(email)}/verify`,
            "PATCH",
            {
                data: {
                    code,
    ...
    async discordLogin(code: string) {
        const res = await this._Request("/user/oauth/discord", "POST", {
            data: {
                code,
            },
        });
    ...
    async recover(identifier: string) {
        return await this._Request("/user/recover", "POST", {
            data: {
                identifier,
            },
        });
